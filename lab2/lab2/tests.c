...
void test_createRepo()
{
    ...
    assert(strcmp(repo->array.data[0].date, "01.04.2023") == 0);
    destroy_r(repo);
    destroy(DA);
}
...
void test_duplicate_repo()
{
    ...
    Repo* repo = createRepo(*DA);
    init_repo(repo);
    Repo new_repo = duplicate_repo(repo);
...
void test_sort_function_quantity()
{
    ...
    for (int i = 0; i < R->array.count; i++)
    {
       ...
       // assert(strcmp(R->array.data[i].date, repo->array.data[i].date) == 1);
    }
    destroy_s(service);
    destroy_r(repo);
    destroy_r(R);
...
void test_search_elements_by_category_s()
{
    ...
    delete_product_r(r, "Cherry", "fruit");
    delete_product_r(r, "Cupcake", "sweets");
   // printf("%d", search_elements_by_category_s(service, "fruit", r, number_of_days));
    assert(search_elements_by_category_s(service, "fruit", r, number_of_days) == 2);
    assert(r->array.count == 1);
    number_of_days = 373;
    delete_product_r(r, "Apple", "fruit");
    assert(search_elements_by_category_s(service, "fruitssss", r, number_of_days) == 0);
...
void test_compute_number_days_between_two_dates()
{
    ...
    char date1[11] = "15.03.2023";
    assert(compute_number_days_between_two_dates(service, date1) == 0);
    char date2[11] = "17.03.2022";
    assert(compute_number_days_between_two_dates(service, date2) == 370);
    char date3[11] = "18.05.2022";
    assert(compute_number_days_between_two_dates(service, date3) == 307);
    char date4[11] = "15.03.2024";
...
{
    char name[100] = "";
    assert(validateProductName(name) == 0);
    char name2[100] = "1Ale24";
    assert(validateProductName(name2) == 0);
    char name3[100] = "Ale";
    assert(validateProductName(name3) == 1);
}

void test_validate_product_quantity()
{
    char quantity[100] = "";
    assert(validateProductQuantity(quantity) == 0);
    char quantity1[100] = "001";
    assert(validateProductQuantity(quantity1) == 0);
    char quantity2[100] = "100A";
    assert(validateProductQuantity(quantity2) == 0);
    char quantity3[100] = "0";
    assert(validateProductQuantity(quantity3) == 0);
    char quantity4[100] = "123";
    assert(validateProductQuantity(quantity4) == 123);
}

void test_validate_product_type()
{
    char type[100] = "fruit";
    assert(validateProductType(type) == 1);
    char type1[100] = "fruits";
    assert(validateProductType(type1) == 0);
}

void test_validate_expiration_date()
{
    char date1[100] = "10.10.11111";
    assert(validateExpirationDate(date1) == 0);
    char date2[100] = "10/10.1111";
    assert(validateExpirationDate(date2) == 0);
    char date3[100] = "10.10/1111";
    assert(validateExpirationDate(date3) == 0);
    char date4[100] = "00.10.2022";
    assert(validateExpirationDate(date4) == 0);
    char date5[100] = "11.00.2022";
    assert(validateExpirationDate(date5) == 0);
    char date6[100] = "11.11.0000";
    assert(validateExpirationDate(date6) == 0);
    char date7[100] = "32.11.2022";
    assert(validateExpirationDate(date7) == 0);
    char date8[100] = "12.13.2022";
    assert(validateExpirationDate(date8) == 0);
    char date9[100] = "12.13.2020";
    assert(validateExpirationDate(date9) == 0);
    char date10[100] = "23.03.2023";
    assert(validateExpirationDate(date10) == 1);
    char date11[100] = "11.A3.20AB";
    assert(validateExpirationDate(date11) == 0);
    char date12[100] = "11.12.2023";
    assert(validateExpirationDate(date12) == 1);
}
